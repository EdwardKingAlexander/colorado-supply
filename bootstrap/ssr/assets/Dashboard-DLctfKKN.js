...
const _sfc_main$6 = {
  ...
  setup(__props) {
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(ssrRenderComponent(unref(Link), {
        href: _ctx.route("store.index"),
        class: "mt-5 inline-flex rounded-md bg-gray-900 px-4 py-2 text-sm font-semibold text-white hover:bg-gray-800"
      }, {
        default: withCtx((_, _push2, _parent2, _scopeId) => {
          if (_push2) {
            _push2(` Browse store `);
...
const _sfc_main$5 = {
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      _push(`<section${ssrRenderAttrs(mergeProps({ class: "grid gap-3 sm:grid-cols-2 xl:grid-cols-5" }, _attrs))}><!--[-->`);
      ssrRenderList(cards, (card) => {
        _push(`<div class="rounded-lg border border-gray-200 bg-white p-4 shadow-sm"><p class="text-xs font-semibold uppercase tracking-wide text-gray-500">${ssrInterpolate(card.label)}</p><p class="mt-2 truncate text-2xl font-semibold text-gray-900">${ssrInterpolate(card.value())}</p></div>`);
      });
      _push(`<!--]--></section>`);
    };
  }
...
const _sfc_main$4 = {
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      _push(`<section${ssrRenderAttrs(mergeProps({ class: "rounded-lg border border-gray-200 bg-white p-4 shadow-sm" }, _attrs))}><div class="grid gap-3 md:grid-cols-6"><label class="text-sm md:col-span-1"><span class="font-medium text-gray-700">Range</span><select class="mt-1 block w-full rounded-md border-gray-300 text-sm"><!--[-->`);
      ...
      _push(`<!--]--></select></label><label class="text-sm md:col-span-1"><span class="font-medium text-gray-700">Start</span><input${ssrRenderAttr("value", form.start_date)} type="date"${ssrIncludeBooleanAttr(form.range !== "custom") ? " disabled" : ""} class="mt-1 block w-full rounded-md border-gray-300 text-sm disabled:bg-gray-100"></label><label class="text-sm md:col-span-1"><span class="font-medium text-gray-700">End</span><input${ssrRenderAttr("value", form.end_date)} type="date"${ssrIncludeBooleanAttr(form.range !== "custom") ? " disabled" : ""} class="mt-1 block w-full rounded-md border-gray-300 text-sm disabled:bg-gray-100"></label><label class="text-sm md:col-span-1"><span class="font-medium text-gray-700">Location</span><select class="mt-1 block w-full rounded-md border-gray-300 text-sm"><option value=""${ssrIncludeBooleanAttr(Array.isArray(form.location_id) ? ssrLooseContain(form.location_id, "") : ssrLooseEqual(form.location_id, "")) ? " selected" : ""}>All</option><!--[-->`);
      ...
      _push(`<!--]--></select></label><label class="text-sm md:col-span-1"><span class="font-medium text-gray-700">Sublocation</span><select class="mt-1 block w-full rounded-md border-gray-300 text-sm"><option value=""${ssrIncludeBooleanAttr(Array.isArray(form.sublocation_id) ? ssrLooseContain(form.sublocation_id, "") : ssrLooseEqual(form.sublocation_id, "")) ? " selected" : ""}>All</option><!--[-->`);
      ssrRenderList(__props.locations, (location) => {
        _push(`<!--[--><!--[-->`);
      ...
      _push(`<!--]--></select></label><div class="flex items-end md:col-span-1"><button type="button" class="w-full rounded-md bg-gray-900 px-4 py-2 text-sm font-semibold text-white hover:bg-gray-800"> Apply </button></div></div></section>`);
    };
  }
};
...
const _sfc_main$3 = {
  ...
  setup(__props) {
    const currency = new Intl.NumberFormat("en-US", { style: "currency", currency: "USD" });
    return (_ctx, _push, _parent, _attrs) => {
      _push(`<section${ssrRenderAttrs(mergeProps({ class: "rounded-lg border border-gray-200 bg-white shadow-sm" }, _attrs))}><div class="border-b border-gray-200 px-5 py-4"><h2 class="text-base font-semibold text-gray-900">Recent orders</h2></div><div class="overflow-x-auto"><table class="min-w-full divide-y divide-gray-200 text-sm"><thead class="bg-gray-50 text-left text-xs font-semibold uppercase tracking-wide text-gray-500"><tr><th class="px-5 py-3">Order</th><th class="px-5 py-3">Date</th><th class="px-5 py-3">Status</th><th class="px-5 py-3">Payment</th><th class="px-5 py-3 text-right">Total</th></tr></thead><tbody class="divide-y divide-gray-100"><!--[-->`);
      ssrRenderList(__props.orders, (order) => {
        _push(`<tr><td class="whitespace-nowrap px-5 py-3 font-medium text-gray-900">${ssrInterpolate(order.order_number)}</td><td class="whitespace-nowrap px-5 py-3 text-gray-600">${ssrInterpolate(order.created_at)}</td><td class="whitespace-nowrap px-5 py-3 text-gray-600">${ssrInterpolate(order.status_label)}</td><td class="whitespace-nowrap px-5 py-3 text-gray-600">${ssrInterpolate(order.payment_status_label)}</td><td class="whitespace-nowrap px-5 py-3 text-right font-medium text-gray-900">${ssrInterpolate(unref(currency).format(order.grand_total))}</td></tr>`);
      });
      _push(`<!--]-->`);
                ...
                createVNode("div", { class: "mx-auto max-w-7xl space-y-6 px-4 py-6 sm:px-6 lg:px-8" }, [
                  createVNode("div", { class: "flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between" }, [
                    createVNode("div", null, [
                      ...
                      createVNode("p", { class: "mt-1 text-sm text-gray-600" }, toDisplayString(__props.filters.start_date) + " through " + toDisplayString(__props.filters.end_date), 1)
                    ]),
