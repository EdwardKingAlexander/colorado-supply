...
const _sfc_main = {
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      _push(`<div${ssrRenderAttrs(mergeProps({ class: "max-w-6xl mx-auto px-4 py-8 space-y-6" }, _attrs))}><div class="flex items-center justify-between"><div><h1 class="text-2xl font-semibold text-gray-900">SAM Opportunities</h1><p class="text-gray-600">Favorited opportunities for your account.</p></div><div class="flex items-center gap-3">`);
      if (!loading.value && favorites.value.length > 0) {
        _push(`<button type="button" class="inline-flex items-center gap-2 rounded-md bg-blue-600 px-4 py-2 text-sm font-medium text-white hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed transition"${ssrIncludeBooleanAttr(exporting.value) ? " disabled" : ""}>`);
        if (!exporting.value) {
      ...
      if (!loading.value && favorites.value.length === 0) {
        ...
        ssrRenderList(favorites.value, (opp) => {
          ...
          _push(`</td><td class="px-4 py-3 text-sm text-gray-700">${ssrInterpolate(opp.agency || "—")}</td><td class="px-4 py-3 text-sm text-gray-700">${ssrInterpolate(opp.posted_date || "—")}</td><td class="px-4 py-3 text-sm text-gray-700">${ssrInterpolate(opp.response_deadline || "—")}</td><td class="px-4 py-3 text-sm text-blue-600">`);
          if (opp.sam_url) {
            _push(`<a${ssrRenderAttr("href", opp.sam_url)} class="hover:underline" target="_blank" rel="noopener noreferrer">View</a>`);
          } else {
            _push(`<span class="text-gray-500">—</span>`);
          }
          _push(`</td><td class="px-4 py-3 text-right"><button type="button" class="${ssrRenderClass([opp.is_favorite ? "border-amber-400 text-amber-600 bg-amber-50" : "border-gray-300 text-gray-700 hover:border-amber-300 hover:text-amber-600", "inline-flex items-center rounded-full border px-3 py-1 text-sm font-medium transition"])}"${ssrIncludeBooleanAttr(toggling.value.has(opp.id)) ? " disabled" : ""}><svg class="${ssrRenderClass([opp.is_favorite ? "fill-amber-500" : "fill-none stroke-current", "h-4 w-4 mr-1"])}" viewBox="0 0 24 24" stroke-width="1.5"><path d="M12 17.27 18.18 21l-1.64-7.03L22 9.24l-7.19-.61L12 2 9.19 8.63 2 9.24l5.46 4.73L5.82 21z"${ssrRenderAttr("fill", opp.is_favorite ? "currentColor" : "none")}${ssrRenderAttr("stroke", opp.is_favorite ? "currentColor" : "currentColor")}></path></svg><span>${ssrInterpolate(opp.is_favorite ? "Favorited" : "Favorite")}</span></button></td><td class="px-4 py-3 text-right"><button type="button" class="inline-flex items-center rounded-md border border-gray-300 px-3 py-1 text-sm text-gray-700 hover:border-blue-300 hover:text-blue-600 transition"> Manage Documents </button></td><td class="px-4 py-3 text-right"><button type="button" class="inline-flex items-center rounded-md border border-gray-300 px-3 py-1 text-sm text-gray-700 hover:border-green-300 hover:text-green-600 transition"> Insights </button></td></tr>`);
          if (openDocsFor.value === opp.id) {
            _push(`<tr class="bg-gray-50"><td colspan="8" class="px-4 py-4"><div class="flex items-start justify-between"><div class="space-y-3 w-full"><div class="flex items-center justify-between"><div><h3 class="text-sm font-semibold text-gray-900">Documents</h3><p class="text-xs text-gray-600">Upload and manage documents for this opportunity.</p></div><div class="flex items-center gap-2"><label class="inline-flex items-center rounded-md border border-gray-300 px-3 py-1.5 text-sm text-gray-700 bg-white cursor-pointer hover:border-blue-300 hover:text-blue-600 transition"><input type="file" class="hidden"${ssrIncludeBooleanAttr(uploading.value.has(opp.id)) ? " disabled" : ""}>`);
            ...
            if (documentsLoading.value.has(opp.id)) {
              ...
              ssrRenderList(documents.value[opp.id], (doc) => {
                _push(`<div class="flex items-center justify-between px-3 py-2"><div><div class="text-sm text-gray-900 font-medium">${ssrInterpolate(doc.original_filename)}</div><div class="text-xs text-gray-600">${ssrInterpolate(doc.mime_type || "Unknown type")} · ${ssrInterpolate(formatSize(doc.size_bytes))} · `);
                if (doc.uploaded_at) {
                ...
                _push(`<button type="button" class="text-sm text-red-600 hover:text-red-700 disabled:opacity-50"${ssrIncludeBooleanAttr(uploading.value.has(doc.id)) ? " disabled" : ""}> Delete </button></div></div>`);
              });
              _push(`<!--]--></div>`);
            }
            _push(`</div></div></td></tr>`);
          ...
          if (insightsOpen.value === opp.id) {
            _push(`<tr class="bg-gray-50"><td colspan="8" class="px-4 py-4"><div class="space-y-3"><div class="flex items-center justify-between"><div><h3 class="text-sm font-semibold text-gray-900">Insights</h3><p class="text-xs text-gray-600">Ask a question about this opportunity or its documents.</p></div></div><div class="flex items-center gap-2"><input${ssrRenderAttr("value", insightsQuestion.value)} type="text" class="w-full rounded-md border border-gray-300 px-3 py-2 text-sm focus:border-blue-400 focus:ring-2 focus:ring-blue-200" placeholder="Ask a question..."><button type="button" class="rounded-md bg-blue-600 px-3 py-2 text-sm font-medium text-white hover:bg-blue-700 disabled:opacity-50"${ssrIncludeBooleanAttr(insightsLoading.value.has(opp.id)) ? " disabled" : ""}>`);
        ...
        if (pagination.value.last_page > 1) {
          _push(`<div class="flex items-center justify-between border-t border-gray-200 bg-white px-4 py-3"><div class="flex items-center gap-2"><button type="button" class="rounded-md border border-gray-300 px-3 py-1.5 text-sm text-gray-700 hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed"${ssrIncludeBooleanAttr(!pagination.value.prev_page_url) ? " disabled" : ""}> Previous </button><button type="button" class="rounded-md border border-gray-300 px-3 py-1.5 text-sm text-gray-700 hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed"${ssrIncludeBooleanAttr(!pagination.value.next_page_url) ? " disabled" : ""}> Next </button></div><div class="text-sm text-gray-600"> Page ${ssrInterpolate(pagination.value.current_page)} of ${ssrInterpolate(pagination.value.last_page)}</div></div>`);
        } else {
          _push(`<!---->`);
        }
        _push(`</div>`);
