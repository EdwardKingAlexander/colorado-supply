...
return [
    ...
    // Conservative default: full APM tracing is out of scope for this phase
    // (error capture only), so this stays low-cost by default.
    'traces_sample_rate' => env('SENTRY_TRACES_SAMPLE_RATE') === null ? 0.1 : (float) env('SENTRY_TRACES_SAMPLE_RATE'),

    // Strips Authorization/Cookie headers and known-sensitive fields
    // (passwords, tax IDs, payment tokens, etc.) from request data and
    // breadcrumbs before the event ever leaves the server. Referenced as
    // an array callable (not a Closure) so this file stays safe to run
    // through `php artisan config:cache`.
