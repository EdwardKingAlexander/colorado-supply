    ...
    'backup' => [
        ...
        'source' => [
            'files' => [
                 ...
                 */
                // Only non-regeneratable uploaded files, not the whole
                // codebase (already in git) or rebuildable cache/log
                // output — keeps backup size/cost sane. Covers both the
                // 'local' and 'public' disks (storage/app/private and
                // storage/app/public): contract documents, SAM
                // opportunity documents, and parsed document artifacts.
                'include' => [
                    storage_path('app'),
                ],
        ...
        'destination' => [
             ...
             *
             * Kept on 'local' until real off-server (S3) credentials exist
             * in .env — a backup that only lives on the same server it's
             * protecting isn't a real off-server backup. Switch this (and
             * 'monitor_backups' below) to 's3' once AWS_ACCESS_KEY_ID /
             * AWS_SECRET_ACCESS_KEY / AWS_BUCKET are populated.
             */
            'disks' => [
                'local',
            ],
