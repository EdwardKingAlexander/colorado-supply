...
{{--
    Workaround for an upstream Filament v4 bug: the `x-float` teleport+flip
    combination miscalculates the topbar user-menu's position, rendering it
    with a negative `top` above the viewport instead of below the trigger.
    Confirmed on filament/filament 4.11.8 (latest stable v4). See
    https://github.com/filamentphp/filament/issues/18648 for the related
    teleport/flip interaction bug. This CSS override forces the panel below
    the fixed topbar without touching Alpine's open/close logic, which broke
    when :teleport or :flip were toggled off directly.
--}}
@once
    <style>
        .fi-user-menu-panel--topbar {
        ...
        \Filament\Support\prepare_inherited_attributes($attributes)
            ...
            ->when(
                ...
                fn ($attributes) => $attributes->merge([
                    'x-init' => '$nextTick(() => $refs.panel?.classList.add(\'fi-user-menu-panel--topbar\'))',
                ], escape: false),
            )
    "
>
